...
namespace BRIDGEFINANCE
{
    public partial class DailySales : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (decimal.TryParse(amt.Text, out d))
            {
                ...
                if (choice == DialogResult.Yes)
                {
                    using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(helper.connectproduct("BRIDGEDB")))
                    {
                        connection.Query($"INSERT INTO statement VALUES('{fromY.Text}/{fromM.Text}/{fromD.Text}', 'DEPOSIT', '{refNo.Text}', '{description.Text}', '{amt.Text}', '{account}')");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult choice = MessageBox.Show($"Approve Deposit?", "CONFIRMATION", MessageBoxButtons.YesNo);
                if (choice == DialogResult.Yes)
                {
                    using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(helper.connectproduct("BRIDGEDB")))
                    {
                        connection.Query($"INSERT INTO dailysales VALUES('{fromY.Text}/{fromM.Text}/{fromD.Text}', '{cashRegister.Text}', '{cashbook.Text}', '{juice.Text}', '{cheque.Text}', '{card.Text}', '{credit.Text}', '{lb.Text}', '{charity.Text}', '{other.Text}', '{totalDaily.Text}', '{comboBox1.Text}')");
        ...
        {
            refNo.Text = $"DS {fromY.Text}.{fromM.Text}.{fromD.Text}";
        }
